...
(function() {
    ...
    function errorHandler() {
        if (!document.querySelector('#zemez-core-error')) {
            ...
            node.setAttribute('style', 'position: fixed; bottom: 1vh; left: 1vw; z-index: 1000; max-width: 98vw; padding: 10px 15px; border-radius: 4px; font-family: monospace; color: white;');
            node.innerText = '';
        ...
        for (let key in this.registry) {
            ...
            nodes.forEach((node) => {
                if (!node.ZemezComponent || !node.ZemezComponent[entry.name]) {
                    // Component creation
                    let component = new ZemezComponent(merge({
                        owner: this,
                        entry: entry,
                        node: node
                    }, entry));
